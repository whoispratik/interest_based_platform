          ...
          <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
             ...
             <div class="flex gap-3">
                 <button type="submit" class="mt-6 btn-primary disabled:cursor-not-allowed" :disabled="utilityStore.isProcessing || isFetching">
                     <span v-if="!utilityStore.isProcessing || !isFetching">Post</span>
                     <EyeLoader v-else></EyeLoader>
                    </button>
                    <button type="reset" class="mt-6 btn-danger" @click="form.reset()">
                        clear
                    </button>
                </div>
      </form>
...
const { form, categoryApiCall, isFetching } = useCategoryApiDealing();
</script>